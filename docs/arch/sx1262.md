...
typedef struct sx1262_s {
    ...
    void (*rx_done)(sx1262_t *p, const uint8_t *payload, size_t len,
                    int8_t rssi, int8_t snr_quarter_db);
    void (*tx_done)(sx1262_t *p);
    void (*rx_timeout)(sx1262_t *p);
    void (*tx_timeout)(sx1262_t *p);

#ifdef SX1262_WITH_LOGGING
    void (*logger)(const char *buf, int len);  // compiled out when disabled
#endif
} sx1262_t;
```
...
|---|---|---|
| `sx1262_set_rx_done_callback()` | `p->rx_done` | `sx1262_service_rx()` on RxDone IRQ |
| `sx1262_set_tx_done_callback()` | `p->tx_done` | `sx1262_service_tx()` on TxDone IRQ |
| `sx1262_set_rx_timeout_callback()` | `p->rx_timeout` | `sx1262_service_rx()` on Timeout IRQ |
| `sx1262_set_tx_timeout_callback()` | `p->tx_timeout` | `sx1262_service_tx()` on Timeout IRQ |
| `sx1262_set_logger_callback()` | `p->logger` | Internally throughout the driver (only when `SX1262_WITH_LOGGING`) |

The `rx_done` callback receives a pointer to the received payload that is
...
callbacks (`ota_tx_init()` registers `tx_done`; `ota_rx_init()` registers
`rx_done`) and then arms the radio:
...
alt RxDone
  ...
  svc -> cb : rx_done(payload, len, rssi, snr)
else TxDone
  svc -> cb : tx_done()
else Timeout
  svc -> cb : rx_timeout() or tx_timeout()
end
