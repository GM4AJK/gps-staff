...
void test_sx1262_hello(sx1262_t *p)
{
	...
	if (status != HAL_OK) {
		logger_log("sx1262: reset failed: %d\r\n", status);
		return;
	}

	status = sx1262_get_status(p, &value);
	if (status != HAL_OK) {
		logger_log("sx1262: get status failed: %d\r\n", status);
		return;
	}
	...
	logger_log("sx1262: status=0x%02X (chip mode=%u, cmd status=%u)\r\n", value, chip_mode, cmd_status);
}
...
void test_sx1262_config(sx1262_t *p)
{
	...
	if (status != HAL_OK) {
		logger_log("sx1262: set packet params failed: %d\r\n", status);
		return;
	}

	status = sx1262_set_buffer_base_address(p, 0, 0);
	if (status != HAL_OK) {
		logger_log("sx1262: set buffer base address failed: %d\r\n", status);
		return;
	}
	...
	if (status != HAL_OK) {
		logger_log("sx1262: set pa config failed: %d\r\n", status);
		return;
	}

	status = sx1262_set_tx_params(p, 0, SX1262_RAMP_200U);
	if (status != HAL_OK) {
		logger_log("sx1262: set tx params failed: %d\r\n", status);
		return;
	}
	...
	if (status != HAL_OK) {
		logger_log("sx1262: set dio irq params failed: %d\r\n", status);
		return;
	}
	...
	logger_log("sx1262: configured LoRa @ 434.000MHz, SF7/BW125/CR4_5, preamble=8 explicit CRC, %u-byte payload, 0dBm\r\n", (unsigned)SX1262_MAX_PAYLOAD_LEN);
}
...
void test_sx1262_config_gfsk(sx1262_t *p)
{
	...
	if (status != HAL_OK) {
		logger_log("sx1262: config gfsk failed: %d\r\n", status);
		return;
	}
	...
	logger_log("sx1262: configured GFSK @ 434.000MHz, 50kbps/25kHz dev/BW117.3kHz, %u-byte fixed CRC16, 0dBm\r\n", (unsigned)SX1262_MAX_PAYLOAD_LEN);
}
...
void test_sx1262_tx_start(sx1262_t *p)
{
	...
	if (status != HAL_OK) {
		logger_log("sx1262: tx write buffer failed: %d\r\n", status);
		return;
	}

	status = sx1262_set_tx(p, SX1262_RX_TX_TIMEOUT_NONE);
	if (status != HAL_OK) {
		logger_log("sx1262: set tx failed: %d\r\n", status);
		return;
	}
}
...
void test_sx1262_rx_start(sx1262_t *p)
{
	...
	if (status != HAL_OK) {
		logger_log("sx1262: set rx failed: %d\r\n", status);
		return;
	}

	logger_log("sx1262: rx armed, cyc=%lu\r\n", (unsigned long)DWT->CYCCNT);
}
