...
SPI bring-up driver for the BNO085 9-DOF IMU: handle initialization, hardware
reset sequence, and a single SHTP packet read used to confirm the SPI link is
working.
...
The driver SHALL provide `bno085_bringup()`, which begins by pulsing `RST`
low for `BNO085_RESET_PULSE_MS` then high, twice in succession, waiting
`BNO085_STARTUP_T1_MS` after each pulse before doing anything else - per
datasheet 6.5.3 "Startup timing", `INT` is undefined for this period (`t1`,
Internal Initialization) after `RST` is released. A single reset pulse does
not reliably bring the device up following a power cycle; a second pulse is
required.

#### Scenario: Reset pulse is issued
- **WHEN** `bno085_bringup()` is called on a handle initialized via
  `bno085_init()`
- **THEN** `RST` is pulsed low for `BNO085_RESET_PULSE_MS` then high, and the
  driver waits `BNO085_STARTUP_T1_MS` without sampling `INT`
- **AND** this pulse-and-wait is repeated a second time before any `INT`
  polling begins

### Requirement: Wait for data-ready and read SHTP packet
After the second `RST` release and its `BNO085_STARTUP_T1_MS` wait,
