/*
 * ota.h -- RTCM3 over-the-air chunking module (base TX side)
 *
 ...
 *
 * ---------------------------------------------------------------------------
 * OTA PACKET FORMAT  (OTA_PACKET_SIZE = 255 bytes, fixed, matching the
 ...
 *                       once per complete RTCM3 frame handed to ota_push_frame()
 *   Byte 2   chunk_idx  0-based index of this chunk within the frame
 *   Byte 3   chunk_count total number of chunks for this frame
 *   Byte 4   data_len   number of valid RTCM3 bytes in data[] (1-OTA_DATA_SIZE);
 *                       the last chunk may be short, padded with zeros to fill
 *                       the fixed packet length
 *   Bytes 5-254  data   RTCM3 frame bytes (OTA_DATA_SIZE = 250 bytes per chunk)
 *
 * ---------------------------------------------------------------------------
 * MEMORY MODEL
 * ---------------------------------------------------------------------------
 * ota_push_frame() performs a memcpy of the incoming frame into ota_t.frame_buf,
 ...
 *        ota_init(&ota, &sx1262);
 *
 *    ota_init() registers ota_tx_done() as the sx1262 TX-done callback
 ...
 *
 *        ota_push_frame(&ota, frame_ptr, frame_len);
 *
 ...
 *    ota_tx_done() callback, which queues the next chunk automatically.
