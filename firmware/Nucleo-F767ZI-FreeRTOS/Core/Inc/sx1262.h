 ...
 * responsibility (e.g. ota_tx_init() registers its own on_tx_done callback via
 * sx1262_set_tx_done_callback() after this returns).
 *
 * Returns HAL_OK on success, or the HAL_StatusTypeDef of the first step that
 * fails. Callers should treat any non-HAL_OK return as a fatal hardware fault.
 */
