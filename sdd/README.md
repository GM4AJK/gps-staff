...
| [features/00011-RoverWorking.html](features/00011-RoverWorking.html) | Rover — Working Screen: live WGS84 position (7 dp lat/lon, 3 dp height MSL), fix quality block (No Fix / RTK Float / RTK Fix with colour coding), hAcc/vAcc, RF corrections status (RSSI/SNR/fps), session strip, Capture Point button (enabled on RTK Fix only); three dev states |
| [features/00012-PointCapture.html](features/00012-PointCapture.html) | Rover — Point Capture: N-sample averaging (configurable dwell, RTK Fix only), running mean lat/lon/height/hAcc/vAcc, editable point label (defaults to P004), fix-lost pause/resume, Confirm & Save / Discard; three dev states (Averaging / Fix Lost / Complete) |
